...
class _CameraPageState extends State<CameraPage> with WidgetsBindingObserver {
  ...
  void navigation(dynamic order) {
    ...
    if (area.length == 2) {
      ...
      print("==");
      print(MRZ.parseThreeLines(area[0].text, area[1].text, area[2].text));
      information =
          MRZ.parseThreeLines(area[0].text, area[1].text, area[2].text);
      information.lines = '${area[0].text}\n${area[1].text}\n${area[2].text}';
    }
    String? id ;
     id = area.first.text.substring(15 , 27);
    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => ResultPage(information: information , ID: (id ?? ''),),
        ));
  }
